...
const config = {
  ...
  db: {
    mysql: {
      ...
      database: process.env.MYSQL_DATABASE || 'node-template',
    ...
    postgres: {
      ...
      database: process.env.POSTGRES_DATABASE || 'node-template',
      user: process.env.POSTGRES_USERNAME || 'node-template',
    ...
    sqlite: {
      client: 'sqlite3',
      connection: {
        filename: process.env.SQLITE_DATABASE || './node-template.db'
      }
    },
    mongodb: {
      client: 'mongodb',
      url: process.env.MONGODB_CONNECTION_URL || 'mongodb://localhost:27017/node-template',
      host: process.env.MONGO_HOST || 'localhost',
      port: parseInt(<string>process.env.MONGO_PORT) || 27017,
      user: process.env.MONGO_USERNAME || 'node-template',
      password: process.env.MONGO_PASSWORD || '',
      database: process.env.MONGO_DATABASE || 'node-template',
  ...
  rabbitmq: {
    ...
    port: parseInt(<string>process.env.RABBITMQ_PORT) || 5672,
